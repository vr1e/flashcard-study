...
class StudySession {
    ...
    endSession(): void {
        ...
        // Update summary
        document.getElementById('summary-cards')!.textContent = this.cards.length.toString();
        document.getElementById('summary-time')!.textContent = this.formatTime(totalTime);
    }
...
async function startStudySession(deckId: number): Promise<void> {
    currentSession = new StudySession(deckId);
...
document.addEventListener('keydown', (event: KeyboardEvent) => {
    ...
    if (event.code === 'Escape') {
        ...
        if (exitLink && confirm('Are you sure you want to exit this study session?')) {
